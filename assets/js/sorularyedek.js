...
function randomNum() {
    return Math.floor(Math.random() * 100) + 1; // 1 ile 100 arasında rastgele bir sayı üretir
}
...
function soruSor(islem) {
   ...
   // kısmı bu
    ...
    var soruMetni = sayi1 + " " + islem + " " + sayi2 + " = ?";
    yanlisCevaplar[0] = dogruCevap - Math.floor(Math.random() * 10) + 1; // dedim ki doğru cevaba yakın olsun ama - kaç randoom belirle
    yanlisCevaplar[1] = dogruCevap + Math.floor(Math.random() * 10) + 1;
    while (yanlisCevaplar[0] == yanlisCevaplar[1] || yanlisCevap[1] == yanlisCevaplar[2]){
       yanlisCevaplar[1] = dogruCevap + Math.floor(Math.random() * 10) + 1; 
